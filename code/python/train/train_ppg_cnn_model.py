# train_cnn_model.py
# This script trains a CNN model for classifying blood pressure changes from PPG signals.
# The initial release includes a small test dataset with 10 subjects. Full datasets will be released gradually.
...
# ---------------- Model Definition ----------------
class CNN(nn.Module):
    def __init__(self, num_classes, seq_len=875):
        super().__init__()
        self.conv1 = nn.Conv1d(3, 128, kernel_size=9, padding=4)
        self.conv2 = nn.Conv1d(129, 256, kernel_size=5, padding=2)
        self.conv3 = nn.Conv1d(257, 128, kernel_size=3, padding=1)
    ...
    def forward(self, x, sbp):
        ...
        x = torch.cat((x, sbp_feat.unsqueeze(1)), dim=1)
...
def cross_validate(dataset, subject_idx, model_cls, criterion, num_epochs, model_path_base):
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(np.arange(len(dataset)), groups=subject_idx)):
        ...
        model = model_cls(num_classes=3, seq_len=875).to(device)
...
if __name__ == "__main__":
    ...
    datasets = {
        "SBP": ("././output/sbp_trn_30_part{}.h5", os.path.join(model_output_dir, "cnn_sbp"))
        # "DBP": ("././output/dbp_trn_15_part{}.h5", os.path.join(model_output_dir, "cnn_dbp")),
        # "MBP": ("././output/mbp_trn_20_part{}.h5", os.path.join(model_output_dir, "cnn_mbp"))
    }

    for name, (pattern, model_path) in datasets.items():
        print(f"Training {name} model (CNN)...")
        dataset = MemoryEfficientPPGDataset(pattern)
        subject_idx = dataset.get_subject_idx()
        results = cross_validate(dataset, subject_idx, CNN, criterion, num_epochs=200, model_path_base=model_path)
